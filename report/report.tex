...
% ─── Title ─────────────────────────────────────────────────────────
\title{\textbf{BenchPress: Predicting LLM Benchmark Scores\\via Low-Rank Matrix Completion}}
\author{
...
% ─── Abstract ──────────────────────────────────────────────────────
\begin{abstract}
Large language models are evaluated on an ever-growing set of benchmarks, yet no single model is tested on all of them. The resulting data forms a sparse matrix with roughly two-thirds of entries missing. We construct a fully-cited benchmark matrix spanning 83~frontier LLMs across 49~benchmarks (1{,}375 observed scores, 33.8\% fill rate) and develop a matrix completion framework to predict the missing 2{,}692~cells. Through a systematic search over 34~candidate methods, we find that a logit-space blend of benchmark regression and rank-2 SVD---\textbf{BenchPress}---achieves 7.25\% median absolute percentage error (MedAPE) on held-out scores under per-model leave-50\%-out evaluation (3~seeds). The logit transform alone accounts for 70\% of the gain by linearizing ceiling and floor effects on percentage-scale benchmarks. Analysis of the matrix's singular value spectrum on a $31 \times 10$ complete submatrix reveals strong low-rank structure: the first principal component explains 71.3\% of the variance, and the first three capture 90.6\%. A set of just five benchmarks can predict the remaining 44 with ${\sim}$7.8\% MedAPE. We additionally show that Claude~Sonnet~4.5, when given the full matrix as context, achieves competitive MedAPE---surpassing all statistical methods on random holdout---at a cost of \$0.86 per evaluation run.
\end{abstract}
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  \item \textbf{A fully-cited benchmark matrix.} We construct an $83 \times 49$ matrix of LLM benchmark scores, where every entry carries a citation URL to its original source. The matrix covers 21~model providers and 11~benchmark categories, with 1{,}375~observed entries (33.8\% fill rate).
...
\begin{figure}[t]
  ...
  \caption{\textbf{Sparsity pattern of the benchmark matrix.} Blue cells indicate observed scores; white cells are missing. Models (rows) are sorted by number of known scores; benchmarks (columns) by coverage. The matrix is 33.8\% filled, with frontier models evaluated more broadly.}
  \label{fig:sparsity}
\end{figure}
...
For each model with $\geq 8$ known scores, we randomly hide 50\% of its observed scores, train the prediction method on the remaining scores plus all other models' complete data, and predict the hidden cells. We repeat this procedure across 3~random seeds and report the \emph{global} median absolute percentage error (MedAPE) over all held-out cells pooled across seeds.
...
\subsection{BenchPress: The Best Method}
\label{sec:logitsvd}

Our best method, \textbf{BenchPress} (formerly LogitSVD Blend), is a weighted average of two complementary predictors:
\begin{equation}
...
\paragraph{Round 3 (9 methods).} The breakthrough: applying the logit transform to SVD as well, and using SVD-Logit as the blend partner. BenchPress (LogitSVD Blend) at 7.25\% MedAPE (post-audit, 3~seeds) became the final winner.
...
Table~\ref{tab:main} presents the post-audit evaluation for BenchPress under per-model leave-50\%-out evaluation (3~seeds).

\begin{table}[t]
  \centering
  \caption{\textbf{Main results: per-model leave-50\%-out evaluation (3~seeds, post-audit).} BenchPress achieves 7.25\% MedAPE with 94.2\% bimodal accuracy and 99.7\% coverage.}
  \label{tab:main}
  \footnotesize
  \begin{tabular}{@{}lrrrrrrr@{}}
    \toprule
    \textbf{Method} & \textbf{MedAPE} & \textbf{MAE} & \textbf{$\pm$3} & \textbf{$\pm$5} & \textbf{BiAcc} & \textbf{Cov} \\
    \midrule
    \textbf{BenchPress}    & \textbf{7.25} & \textbf{4.71} & \textbf{36.5} & \textbf{51.7} & \textbf{94.2} & \textbf{99.7} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{\textbf{Method comparison.} Per-model MedAPE for all evaluated methods. Lower is better. BenchPress (7.25\%) provides the best accuracy-coverage combination.}
  \label{fig:methods}
\end{figure}
...
The relative improvement from BenchReg+KNN Blend to BenchPress can be decomposed into two independent changes:
...
The SVD spectrum of the $31 \times 10$ mean-centered complete submatrix reveals strong low-rank structure (Table~\ref{tab:spectrum}). The first principal component alone explains 71.3\% of the variance, and the first three capture 90.6\%.

\begin{table}[h]
  \centering
  \caption{\textbf{Singular value spectrum} (31$\times$10 complete submatrix, mean-centered). The matrix exhibits strong low-rank structure.}
  \label{tab:spectrum}
  \begin{tabular}{crr}
    \toprule
    \textbf{Components} & \textbf{Cumulative Var.\ Explained} \\
    \midrule
    1 & 71.3\% \\
    1--2 & 83.0\% \\
    1--3 & 90.6\% \\
...
Figure~\ref{fig:phase} shows how BenchPress accuracy scales with the number of known benchmark scores per model.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  \item \textbf{Biggest gains from 1$\to$5 scores.} MedAPE drops from 12.08\% (1~known score) to 9.23\% (5~known scores). Each additional benchmark narrows the uncertainty about where the model sits in the 2D latent space.
...
These five benchmarks span four categories (reasoning, math, coding, knowledge) and load on both latent factors. Under proper holdout evaluation (random 20\%), this 5-benchmark Ridge predictor achieves ${\sim}7.8\%$ MedAPE compared to BenchPress's 5.95\%. The in-sample figure of 4.8\% MedAPE is optimistic due to train-test leakage.
...
\begin{table}[h]
  \centering
  \caption{\textbf{Claude-as-predictor results} vs.\ statistical baselines (random 20\% holdout, 275~cells, post-audit).}
  \label{tab:claude}
  \small
  \begin{tabular}{@{}lrrrrrrr@{}}
    \toprule
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  \item \textbf{BenchPress edges out Claude on random holdout}, achieving 5.81\% MedAPE vs.\ Claude's 6.08\% on the same post-audit holdout (275~cells). This is notable because the statistical method is zero-cost and deterministic.

  \item \textbf{Claude wins on bimodal accuracy.} Claude~Sonnet~4.5 achieves 92.9\% bimodal accuracy vs.\ BenchPress's 78.6\%. The LLM appears to use world knowledge to better classify models on bimodal benchmarks.

  \item \textbf{Cost-accuracy tradeoff.} At \$0.86 per run, the Claude predictor is inexpensive in absolute terms but infinitely more expensive than the zero-cost statistical methods. For one-off predictions, the LLM approach is practical; for large-scale or repeated evaluation, statistical methods are preferred.
\end{enumerate}
...
Claude Sonnet 4.5's competitive performance as a predictor likely reflects three factors: (1) the LLM has been trained on vast amounts of text about model capabilities, benchmark results, and the AI evaluation landscape; (2) the matrix itself is low-rank, so the prediction task is fundamentally about inferring latent scores from context; and (3) LLMs excel at in-context tabular reasoning when the table fits in the context window (${\sim}$13K tokens per batch).

\subsection{Limitations}

\begin{itemize}[leftmargin=*, itemsep=2pt]
  \item \textbf{33.8\% fill rate.} The matrix is sparse. Additional scores could be mined from model papers, leaderboards, and community evaluations.
