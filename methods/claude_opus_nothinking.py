#!/usr/bin/env python3
"""
Opus 4.6 row-only WITHOUT extended thinking.
Thinking mode wastes 30K tokens on reasoning in some batches (0 predictions).
This version uses standard mode with max_tokens=8192.
"""
...
def main():
    ...
    # ── Run Opus 4.6 row-only (NO THINKING) ──
    print("\n" + "="*100)
    print(f"  Opus 4.6 ROW-ONLY — NO THINKING")
    print("="*100)
    ...
    opus_thinking = None
    try:
        with open(os.path.join(REPO_ROOT, 'results', 'claude_opus_rowonly.json')) as f:
            opus_thinking = json.load(f)
    except Exception:
        pass

    print(f"\n  ALL BENCHMARKS (n={len(test_set)} cells)")
    print(f"  {'Method':<36s} {'MedAPE':>8s} {'MAE':>7s} {'±3pts':>6s} {'±5pts':>6s} {'BiAcc':>6s} {'Cov':>5s}")
    print("  " + "─"*74)
    ...
    if sonnet_results:
        ...
        if sf:
            print(f"  {'Sonnet 4.5 (full-matrix)':<36s} {sf['medape']:>7.2f}% {sf['mae']:>7.2f} "
                  f"{sf['within3']:>5.1f}% {sf['within5']:>5.1f}% {sf.get('bimodal_acc', 0):>5.1f}% {sf['coverage']:>4.1f}%")
        if sr:
            print(f"  {'Sonnet 4.5 (row-only)':<36s} {sr['medape']:>7.2f}% {sr['mae']:>7.2f} "
                  f"{sr['within3']:>5.1f}% {sr['within5']:>5.1f}% {sr.get('bimodal_acc', 0):>5.1f}% {sr['coverage']:>4.1f}%")

    if opus_thinking:
        ot = opus_thinking.get('all', {})
        if ot:
            print(f"  {'Opus 4.6 (row-only, thinking)':<36s} {ot['medape']:>7.2f}% {ot['mae']:>7.2f} "
                  f"{ot['within3']:>5.1f}% {ot['within5']:>5.1f}% {ot.get('bimodal_acc', 0):>5.1f}% {ot['coverage']:>4.1f}%")

    print(f"  {'Opus 4.6 (row-only, NO thinking)':<36s} {m_opus['medape']:>7.2f}% {m_opus['mae']:>7.2f} "
          f"{m_opus['within3']:>5.1f}% {m_opus['within5']:>5.1f}% {m_opus.get('bimodal_acc', 0):>5.1f}% {m_opus['coverage']:>4.1f}%")
    print(f"  {'LogitSVD Blend':<36s} {m_lsvd['medape']:>7.2f}% {m_lsvd['mae']:>7.2f} "
    ...
    print(f"  {'Opus 4.6 (row-only, NO thinking)':<36s} {m_opus_pct['medape']:>7.2f}% {m_opus_pct['mae']:>7.2f} "
          f"{m_opus_pct['within3']:>5.1f}% {m_opus_pct['within5']:>5.1f}% {m_opus_pct.get('bimodal_acc', 0):>5.1f}% {m_opus_pct['coverage']:>4.1f}%")
    print(f"  {'LogitSVD Blend':<36s} {m_lsvd_pct['medape']:>7.2f}% {m_lsvd_pct['mae']:>7.2f} "
    ...
    json_path = os.path.join(REPO_ROOT, 'results', 'claude_opus_nothinking.json')
